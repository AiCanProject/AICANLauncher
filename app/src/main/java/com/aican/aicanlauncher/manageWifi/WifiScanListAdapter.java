package com.aican.aicanlauncher.manageWifi;
...
public class WifiScanListAdapter {
        ...
        if (connectedSSID != null && scannedList != null) {
            ...
            for(ScanResult result : scannedList) {
                if(result.SSID.equals(connectedSSID)) {
                    _connectedNetworkSecure = NetworkCapability.isEncryptionEnabledSupported(result.capabilities).first;
                } else {
                    _dataset.add(result);
                }
            }
            ...
            if(_dataset.get(0) instanceof WifiInfo) {
                ...
                //TODO: flag to indicate only connected network info is changed
                ...
                for(Object exRs : _dataset) {
                    ...
                    if(exRsC.SSID.equals(connectedSSID)) {
                        _connectedNetworkSecure = NetworkCapability.isEncryptionEnabledSupported(exRsC.capabilities).first;
                    } else {
    ...
    public WifiBriefInfo getBriefInfo(int position) {
        ...
        if(item instanceof WifiInfo) {
            ...
            Pair<Boolean, Boolean> netStatus = NetworkCapability.isEncryptionEnabledSupported(result.capabilities);
